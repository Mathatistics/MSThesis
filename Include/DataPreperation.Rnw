...
baseTable$Testrain<-format(baseTable$Date, "%Y")<2014
...
baseTable$l2y.var<-lag(baseTable[,"PerEURO"], 2)
baseTable<-baseTable[-c(1:2),]  # To Remove NA due to introduction of Lagged Variable in Response

## Label Variables in baseTable
labelTable<-read.xls(path.expand(file.path(dirname(dirname(getwd())), "Datasets", "CompleteDataSet.xlsx")), sheet = "ExtDataCode", stringsAsFactors=FALSE)
