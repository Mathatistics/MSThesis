...
\texttt{PerEURO}&=f(\texttt{interest Rate}, \texttt{Trade}, \texttt{Price}, \texttt{Expectation})+\texttt{Error} \nonumber \\
&=\alpha_0 +\alpha_1(\texttt{interest Rate})+\alpha_2(\texttt{Trade}) \nonumber \\
&\hspace{1cm}+\alpha_3(\texttt{Price})+\alpha_4(\texttt{Expectation})+\texttt{Error}
\end{align}
...
The models selected by these criteria when fitted results on few insignificant variables. The plot of the t-value in fig-\ref{fig:cpr2mdlplt} has only one (for $C_p$ criteria) and two ($R^2$adj criteria) are insignificant. With fewer variables than the full model, this model has described the variation almost equally as the the linear model with full variables (table-\ref{tbl:gofSumry}).
...
\subsection{Stepwise procedures based on F-value}
...
vifPlot<-lapply(c("linear","cp.model", "r2.model", "backward"), getPlot)
