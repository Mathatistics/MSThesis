...
<<cpr2mdlplt, echo=FALSE, out.width="0.5\\textwidth", fig.subcap=c("Model selected from Mallows' $C_p$ criteria","Model selected from $R^2$ adjusted criteria"), fig.cap="Best subset model selected by $C_p$ and $R^2$ adjusted criteria. Red and blue bars are significant and insignificant variables respectively. The estimates rounded at 2 decimals are given on top of the bars.", fig.pos='!ht', purl=FALSE>>=
...
<<LMstatCompSetup, echo=FALSE, results='hide'>>=
...
@
<<LMstatComp, echo=FALSE, results='asis', purl=FALSE>>=
