...
<<rpCorPlot, echo=FALSE, fig.height=5, fig.cap="The bars represents the correlation between response variable (\\texttt{PerEURO}) and other predictor variable. The bars are shaded with the p-value for their signifance test performed by \\texttt{cortest} function. The red horizontal line is the critical value at 5 percent level of significance.">>=
